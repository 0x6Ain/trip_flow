        ...
        <div className="flex-1 flex items-center justify-center bg-white">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">
              여행 계획 페이지에서 각 장소의 방문 시간을 설정해주세요
            </p>
            <div className="text-xs text-gray-400 space-y-1">
              ...
              {places.length > 0 && (
                <p className="mt-2">
                  {places.map(p => `${p.name}${p.visitTime ? ` (${p.visitTime})` : ' (시간 없음)'}`).join(', ')}
                </p>
              )}
            </div>
          </div>
      ...
      {hasPlacesWithTime && (
        ...
        <div className="w-[120px] bg-[#FAFAFA] border-r border-[#E5E7EB] flex flex-col">
          ...
          <div className="h-[60px] border-b border-[#E5E7EB] flex items-center justify-center">
            <span className="text-sm font-semibold text-[#6B7280] font-[Inter]">
              Time
            </span>
          </div>
          ...
          {HOURS.map((hour) => (
            <div
              key={hour.value}
              className="h-20 border-b border-[#E5E7EB] flex items-center justify-center"
            >
              <span className="text-xs font-medium text-[#9CA3AF] font-[Inter]">
                {hour.label}
              </span>
            </div>
          ))}
        ...
        <div className="flex-1 flex">
          {DAYS_OF_WEEK.map((day, dayIndex) => {
            ...
            return (
              <div
                key={day}
                className="flex-1 border-r border-[#E5E7EB] last:border-r-0 flex flex-col relative"
              >
                {/* Day Header */}
                <div className="h-[60px] border-b border-[#E5E7EB] flex flex-col items-center justify-center gap-0.5">
                  ...
                  <div className="flex items-center gap-1">
                    {startDate && (
                      <>
                        <span className="text-xs text-[#9CA3AF] font-[Inter]">
                ...
                <div className="flex-1 relative">
                  ...
                  {HOURS.map((hour) => (
                    <div
                      key={hour.value}
                      className="h-20 border-b border-[#E5E7EB]"
                    />
                  ))}
                  ...
                  <div className="absolute inset-0 p-1">
                    {dayPlaces.map((place, idx) => {
                      ...
                      const segment = nextPlace
                        ? getRouteSegment(place, nextPlace)
                        : null;
                        ...
                        if (nextStyle) {
                          ...
                          if (connectionHeight > 0) {
                            connectionElement = (
                              <div
                                onClick={() =>
                                  handleSegmentClick(place, nextPlace, segment)
                                }
                              ...
                              >
                                ...
                                <div className="w-0.5 flex-1 bg-[#F97316] min-h-[8px]" />
                                <span className="text-sm font-semibold text-[#F97316] -mt-1">
                                  ▼
                                </span>
                              </div>
                            );
                      ...
                      return (
                        <div key={place.id}>
                          ...
                          >
                            <span className="text-sm font-semibold text-white font-[Inter] truncate">
                              {place.name}
                            </span>
